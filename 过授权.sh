...
trap exit SIGTSTP
rm -rf $0 /root/Lyun
# mv /usr/bin/tianmuxi /usr/bin/base64
...
☆-Powered by 84db.com 2018 All Rights Reserved                
...
☆-Powered by klutz 2018 All Rights Reserved     
      
☆-交流群：259282245     欢迎你的加入                                    
=====================================================';
HmdLogo='
=====================================================  
...
lyWEB="WEB_t1-1.0.zip";export lyWEB=$lyWEB
...
Host='222.186.139.88/oss/Lyun';export Host=$Host
...
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' "/home/android/Lyun/smali/net/openvpn/openvpn/OpenVPNClient.smali" 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' "/home/android/Lyun/smali/net/openvpn/openvpn/OpenVPNClient\$10.smali" 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' "/home/android/Lyun/smali/net/openvpn/openvpn/OpenVPNClient\$11.smali" 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' "/home/android/Lyun/smali/net/openvpn/openvpn/OpenVPNClient\$13.smali" 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' "/home/android/Lyun/smali/net/openvpn/openvpn/Main2Activity\$MyListener\$1.smali" 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' '/home/android/Lyun/smali/net/openvpn/openvpn/Main2Activity$MyListener.smali' 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' '/home/android/Lyun/smali/net/openvpn/openvpn/MainActivity.smali' 
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' '/home/android/Lyun/smali/net/openvpn/openvpn/update$myClick$1.smali'
sed -i 's/'118.24.208.254:8888'/'${IP}:${webdk}'/g' '/home/android/Lyun/smali/net/openvpn/openvpn/AutoScrollTextView.smali' 
sed -i 's/凌云流量/'$appname'/g' "/home/android/Lyun/res/values/strings.xml"
if [[ $img == 'yes' ]];then
  # 图标
  rm -rf /home/android/Lyun/res/drawable-hdpi-v4/icon.png
  rm -rf /home/android/Lyun/res/drawable-mdpi-v4/icon.png
  rm -rf /home/android/Lyun/res/drawable-xhdpi-v4/icon.png
  wget -q -O /home/android/Lyun/res/drawable-xhdpi-v4/icon.png $urlA
  cp /home/android/Lyun/res/drawable-xhdpi-v4/icon.png /home/android/Lyun/res/drawable-mdpi-v4/icon.png
  cp /home/android/Lyun/res/drawable-xhdpi-v4/icon.png /home/android/Lyun/res/drawable-hdpi-v4/icon.png

  # 启动图
  rm -rf /home/android/Lyun/res/drawable/splash.png
  wget -q -O /home/android/Lyun/res/drawable/splash.png $urlB
fi
...
# APP软件客服
echo
echo -n -e "请输入APP客服QQ [默认:klutzQQ ]"
read adminqq
if [ -z $adminqq ]
then
	adminqq=863963860
...
then 
    img=no
	echo -e "\033[1;34mAPP图标地址已设置为：\033[32m 凌云Logo图标 \033[0m" ;
else
...
then 
    img=no
	echo -e "\033[1;34mAPP启动图地址已设置为：\033[32m 凌云启动图 \033[0m" ;
else
...
给你两分钟时间去群里装个逼,或者去我的博客转转( lsuyi.cn )\033[0m"
echo
...
if [ $NowV != '4.13.7-1.el7.elrepo.x86_64' ];then
  if [ ! -e "/opt/BBR_elrepo_install" ];then
     ...
     grub2-set-default 0
     echo '84db.com'>>/opt/BBR_elrepo_install
...
addn-hosts=/etc/Lyun_hosts
cache-size=102400" >/etc/dnsmasq.conf
echo "请输入拦截规则" >/etc/Lyun_hosts
chmod 0777 /etc/Lyun_hosts
...
echo
echo -n -e "请输入凌云官网网址：[\033[32m 84db.com\033[0m ]："
read key
if [[ $key == '84db.com' ]]
then
	echo
...
if [[ $KSH == 'ipok' ]] ;then
	...
	echo
	echo -e "请输入正版密钥开启安装向导（购买地址:\033[32m http://qk.youkamai.com \033[0m）"
	echo
	echo -n " 请输入卡号： "
	read name
	echo -n " 请输入密码： "
	...
	echo
	sleep 3
	modes=`wget http://27.155.84.216:7799/shell.php?km=${name}":"${code} -O - -q ; echo`;
	echo
if [[ $modes == 'km01' ]] ;then
    echo -e "您的服务器IP已经绑定了永久授权码！可永久搭建！"
	echo
	...
	echo 
    echo -e "  高级密钥:\033[32m 15.8\033[0m 元/永久授权"
	echo -e "  购买地址:\033[32m http://qk.youkamai.com \033[0m"
...
exit 0
  ...
  sed -i 's/dlapp=863963860/'dlapp=$dlapp'/g' "/home/android/DlAPP/smali/net/openvpn/openvpn/OpenVPNClient.smali" 
  sed -i 's/dlapp=863963860/'dlapp=$dlapp'/g' "/home/android/DlAPP/smali/net/openvpn/openvpn/OpenVPNClient\$13.smali" 
  ...
  source /etc/Lyun/Lyun.conf
  #echo -e '正在为您备份用户数据...\n'
  #mysqldump -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME openvpn > /mnt/openvpn.sql
  #echo -e '正在为您备份卡密数据...\n'
  #mysqldump -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME auth_kms > /mnt/auth_kms.sql
  #echo -e '正在为您备份线路数据...\n'
  #mysqldump -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME line > /mnt/line.sql
  #echo -e '正在为您备份排行榜数据...\n'
  #mysqldump -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME top > /mnt/top.sql
  echo -e '\n正在为您备份系统文件...\n'
  cp /Data/wwwroot/Lyun/Data/config.php /mnt/config_one.php.bak
  cp /Data/wwwroot/Lyun/Lyws/config.php /mnt/config_two.php.bak
  mv /Data/wwwroot/Lyun/mysql_* /mnt
  rm -rf /Data/wwwroot/Lyun/*
  echo -e '正在更新WEB文件...\n'
  cd /Data/wwwroot/Lyun 
  wget -q ${web}$Host/$lyWEB #${web}$Host/$lyWEB 
  unzip -q $lyWEB 
  rm -rf $lyWEB 
  rm -rf /Data/wwwroot/Lyun/Data/config.php 
  rm -rf /Data/wwwroot/Lyun/Lyws/config.php 
  mv /mnt/mysql_*   /Data/wwwroot/Lyun/
  mv /mnt/config_one.php.bak /Data/wwwroot/Lyun/Data/config.php 
  mv /mnt/config_two.php.bak /Data/wwwroot/Lyun/Lyws/config.php 
  cd /mnt 
  wget -q ${web}$Host/gxsql.sql
  sed -i 's/www.tianmuxi.ooo:8888/'$IP:$webdk'/g' /mnt/gxsql.sql
  sed -i 's/www.tianmuxi.ooo/'$IP'/g' /mnt/gxsql.sql
  mysql -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME < /mnt/gxsql.sql
  echo -e '正在更新系统文件...\n'
  cd /etc/openvpn 
  ...
  rm -rf /etc/Lyun/*.sh
  mv /etc/openvpn/K/*.sh /etc/Lyun
  chmod 0777 /etc/Lyun/* 
  rm -rf /bin/lingyun
  wget -q ${web}$Host/lingyun
  mv lingyun /bin/lingyun
  chmod 0777 /bin/*
  vpn
  chmod 0777 /Data/wwwroot/Lyun/Online/*
  #echo -e '正在为您恢复用户数据...\n'
  #mysql -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME < /mnt/openvpn.sql
  #echo -e '正在为您恢复卡密数据...\n'
  #mysql -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME < /mnt/auth_kms.sql
  #echo -e '正在为您恢复线路数据...\n'
  #mysql -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME < /mnt/line.sql
  #echo -e '正在为您恢复排行榜数据...\n'
  #mysql -u$Lyun_USER -p$Lyun_PASS $Lyun_NAME < /mnt/top.sql
  clear
