...
#2019.10.10 修复centos7下无法安装php环境
rpm -Uvh https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
#
rpm -Uvh https://mirror.webtatic.com/yum/el7/webtatic-release.rpm
yum -y install php70w php70w-fpm php70w-bcmath php70w-cli php70w-common php70w-dba php70w-devel php70w-embedded php70w-enchant php70w-gd php70w-imap php70w-ldap php70w-mbstring php70w-mcrypt php70w-mysqlnd php70w-odbc php70w-opcache php70w-pdo php70w-pdo_dblib php70w-pear.noarch php70w-pecl-apcu php70w-pecl-apcu-devel php70w-pecl-imagick php70w-pecl-imagick-devel php70w-pecl-mongodb php70w-pecl-redis php70w-pecl-xdebug php70w-pgsql php70w-xml php70w-xmlrpc php70w-intl php70w-mcrypt --nogpgcheck php-fedora-autoloader php-php-gettext php-tcpdf php-tcpdf-dejavu-sans-fonts php70w-tidy
...
fasapkipname=$(curl -s https://dir.smwlm.com/fasvpn/ip.php)
fi
...
echo
read -p "请设置后台密码(默认随机): " fasadminpasswd
...
echo
read -p "请设置数据库地址(默认随机,禁用phpMyAdmin): " fassqlip
if [ -z "$fassqlip" ]; then
fassqlip=$(
date +%s%N | md5sum | head -c 20
echo
...
read -p "请设置APP解析地址(可输入域名或IP，不带http://): " fasapkipname
if [ -z "$fasapkipname" ]; then
fasapkipname=$(curl -s http://dir.smwlm.com/fasvpn/ip.php)
fi
echo -e "已设置APP解析地址为:\033[32m "$fasapkipname"\033[0m"
...
echo "请稍等..."
sleep 2
IP=$(curl -s https://dir.smwlm.com/fasvpn/ip.php)
clear
sleep 1
echo
