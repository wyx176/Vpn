...
lyWEB="WEB_t1-2.0.zip";export lyWEB=$lyWEB
...
# APP软件图标
echo
echo -n -e "请输入APP图标地址 [默认:刺激Logo图标 ]"
read urlA
if [ -z $urlA ]
then 
    img=no
	echo -e "\033[1;34mAPP图标地址已设置为：\033[32m 刺激Logo图标 \033[0m" ;
else
...
# APP启动图
echo
echo -n -e "请输入APP启动图地址 [默认:刺激启动图 ]"
read urlB
if [ -z $urlB ]
then 
    img=no
	echo -e "\033[1;34mAPP启动图地址已设置为：\033[32m 刺激启动图 \033[0m" ;
else
...
if [ $NowV != '4.13.7-1.el7.elrepo.x86_64' ];then
  if [ ! -e "/opt/BBR_elrepo_install" ];then
     ...
     grub2-set-default 0
     echo 'ly.52hgy.cn'>>/opt/BBR_elrepo_install
...
#KSH=`curl -s http://27.155.84.216:7799/shell.php`;
KSH=ipok
if [[ $KSH == 'ipok' ]] ;then
	echo 
	...
	echo
	sleep 3
	modes=`wget http://118.89.65.157/shell.php?km=${name}":"${code} -O - -q ; echo`;
	echo
if [[ $modes == 'km01' ]] ;then
    echo -e "您的服务器IP已经绑定了永久授权码！可永久搭建！"
	echo
	...
	echo -e "  购买地址:\033[32m http://www.510ka.com/links/E91C9B0F \033[0m"
...
exit 0
  ...
  sed -i 's/dlapp=1134107881/'dlapp=$dlapp'/g' "/home/android/DlAPP/smali/net/openvpn/openvpn/OpenVPNClient.smali" 
  sed -i 's/dlapp=1134107881/'dlapp=$dlapp'/g' "/home/android/DlAPP/smali/net/openvpn/openvpn/OpenVPNClient\$13.smali" 
